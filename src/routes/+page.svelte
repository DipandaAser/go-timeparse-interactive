<script lang="ts">
  ...
  import LayoutRender from "../components/LayoutRender.svelte";

  let examples: string[] = [
    "Jan 02, 2006, 3:04:05 PM MST",
    "02/01/2006, 15:04",
    "3:04:05:.000",
    "3:04:05 PM MST",
  ];
  let globalLayout = examples[0];
  let showToast = false;
  ...
  function replaceGlobalLayout(layout: string) {
    globalLayout = layout;
  }

  function copyLayoutToClipboard() {
    navigator.clipboard.writeText(globalLayout).then(() => {
      toastMessage = "Layout copied in clipboard";
      showToast = true;
      setTimeout(() => {
        showToast = false;
      }, toastTimeout);
...
<div class="mx-auto container flex flex-col p-4">
  ...
  >
    <h2 class="text-2xl text-gray-200 font-bold select-none">Now</h2>
    <div
      class="px-3 py-1 text-center text-3xl text-gray-300 font-medium border-b-2 border-gray-900"
    >
      <LayoutRender layout={globalLayout} />
    </div>
    <h2 class="text-2xl text-gray-200 font-bold select-none">Layout</h2>
    <div class="flex space-x-5">
      ...
      />
      <button class="hidden md:block" on:click={copyLayoutToClipboard}>
        <svg
  ...
  <div class="p-8">
    ...
    <div class="mb-4 w-full grid grid-cols-2 gap-4">
      {#each examples as example}
        <FormatExample {replaceGlobalLayout} layout={example}></FormatExample>
      {/each}
    </div>
